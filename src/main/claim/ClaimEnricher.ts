...
function buildFallbackEnrichPrompt(
  claim: string,
  contextText: string,
  claimNumber: number,
  searchInstruction = '',
...
[사용자 보조 참고자료]
${additionalContext || '(사용자 보조 참고자료 없음)'}

[보조 참고자료 사용 규칙]
사용자 보조 참고자료는 청구항의 모호한 용어를 해석하고 검색어를 구체화하기 위한 근거로 사용할 수 있습니다.
다만 보조 참고자료에 없는 내용을 일반 지식으로 새로 추가하지 마십시오.
...
function parseEnrichResponse(
  ...
  try {
    ...
    warnings.push(`청구항 ${claimNumber}: 프롬프트 응답을 JSON으로 파싱하지 못했습니다. 구성요소·가중치·검색쿼리를 사용할 수 없어 청구항 원문으로 폴백합니다.`)
    return { claim: minimalEnrichment(originalClaim, claimNumber, isIndependent, parentClaimNumber), warnings }
  }
  ...
  const components: ClaimComponent[] = rawComponents.map((c) => {
    ...
    return {
      ...
      importance:        clampImportance(comp['importance']),
  ...
  if (components.length === 0) {
    warnings.push(`청구항 ${claimNumber}: 응답에 'components' 필드가 없어 구성요소·가중치를 사용할 수 없습니다. 내부 기본값으로 폴백합니다.`)
    components.push({
  ...
  // searchQueries 없음 → 폴백 경고
  if (searchQueries.length === 0) {
    warnings.push(`청구항 ${claimNumber}: 응답에 'searchQueries' 필드가 없어 검색쿼리를 사용할 수 없습니다. 청구항 원문으로 폴백합니다.`)
  }
...
function minimalEnrichment(
  ...
  return {
    ...
    components: [{ name: `청구항 ${num}`, importance: 3, technicalConcepts: [], functionalRoles: [], synonyms: [], effects: [], supportingText: '' }],
    overallPurpose: '',
    technicalDomain: '',
...
function clampImportance(value: unknown): 1 | 2 | 3 {
...
export class ClaimEnricher {
  ...
  constructor(db?: DatabaseManager) {
    this.promptRegistry = db ? new PromptRegistry(db) : undefined
  }
  ...
  async enrich(params: EnrichClaimsParams): Promise<EnrichmentResult> {
    const { patentStructure, chunks, targetClaimNumbers, searchInstruction, additionalContext } = params
    ...
    for (let i = 0; i < claimsToProcess.length; i++) {
      ...
      // ── LLM call ──────────────────────────────────────────────────────────
      const prompt = this.buildPromptFromSettings(
        claim,
        contextText,
        claimNum,
        patentStructure,
      ...
      const systemPrompt = '당신은 특허 명세서와 청구항을 분석하는 전문가입니다. 사용자가 설정한 프롬프트의 지시를 최우선으로 따르십시오.'
      ...
      try {
        ...
        allWarnings.push(`청구항 ${claimNum}: LLM 호출 실패 — 구성요소·가중치·검색쿼리를 사용할 수 없습니다. 청구항 원문으로 폴백합니다.`)
        enrichedClaims.push(minimalEnrichment(claim, claimNum, isIndep, parentNum))
        continue
      }

      // ── Parse response ────────────────────────────────────────────────────
      const { claim: enriched, warnings } = parseEnrichResponse(
        rawResponse, claim, claimNum, isIndep, parentNum, evidenceIds
      )
      if (warnings.length > 0) {
        console.warn('[ClaimEnricher] 파싱 경고:', warnings)
  ...
  private buildPromptFromSettings(
    claim: string,
    contextText: string,
    claimNumber: number,
    ...
    const template = this.promptRegistry?.getActive('claims_analysis')
    if (!template) {
      return buildFallbackEnrichPrompt(claim, contextText, claimNumber, searchInstruction, additionalContext)
    }

    const references = [
      contextText ? `[명세서/참조 문맥]\n${contextText}` : '',
      additionalContext ? `[사용자 보조 참고자료]\n${additionalContext}` : '',
      searchInstruction ? `[유사특허조회 검색 전략]\n${searchInstruction}` : '',
    ].filter(Boolean).join('\n\n---\n\n')

    return this.promptRegistry!.render(template, {
      claims_text: claim,
      claims: claim,
      prior_art_references: references,
      invention_title: patentStructure.title,
      invention_description: patentStructure.rawText.slice(0, 8_000),
      technical_field: patentStructure.sections.find((section) => section.type === 'technical_field')?.text ?? '',
    })
  }
}
