...
function extractPatentDate(rawText: string): string | undefined {
  // 서지사항은 보통 앞 12,000자 이내에 위치 (넉넉하게 잡음)
  const headerRaw = rawText.slice(0, 12000)

  // 한국 PDF에서 문자 사이에 공백이 삽입되는 경우 처리
  // 예: "( 3 0 )" → "(30)", "우 선 권 주 장" → "우선권주장"
  const header = headerRaw
    .replace(/\(\s*(\d)\s*(\d)\s*\)/g, '($1$2)')   // (3 0) → (30), (3 2) → (32)
    .replace(/\(\s*(\d)\s*\)/g, '($1)')             // (2 2) → (22) 등 3자리 이하
    .replace(/(우|선|권|주|장|출|원|일|자|심|사|청|구)\s+(?=[가-힣])/g,
      (_, c) => c)                                  // 한글 단어 내 공백 일부 제거
  ...
  function findDate(keyword: string, searchLen = 200): string | undefined {
    ...
    if (isValidYear(year)) return normalize(m[1], m[2], m[3])
    return undefined
  }

  // 1순위: 우선권주장 날짜 — WIPO (32)=우선권날짜, (30)=우선권정보통합, 텍스트 키워드
  const d_priority =
    findDate('(32)') ??
    findDate('(30)') ??
    findDate('우선권주장일') ??
    findDate('우선권주장')
  if (d_priority) { dbg('날짜 추출 성공 [1순위 우선권주장]:', d_priority); return d_priority }

  // 2순위: 심사청구일자 / 출원일자 — WIPO 서지코드 (22)
  const d22 =
    findDate('(22)') ??
    findDate('심사청구일자') ??
    findDate('심사청구일') ??
    findDate('출원일자') ??
    findDate('출원일')
  if (d22) { dbg('날짜 추출 성공 [2순위 심사청구/출원일]:', d22); return d22 }
  ...
  dbg('  헤더 샘플 (처음 500자):', headerRaw.slice(0, 500))
  return undefined
}
...
export function parsePatentStructure(
  ...
  let claims = claimsRaw.trim() ? splitClaims(claimsRaw) : []

  // 섹션 헤더를 인식하지 못했을 때 전체 텍스트에서 "청구항 N" 패턴 폴백
  if (claims.length === 0 && rawText.length > 0) {
    dbg('섹션 파싱으로 청구항 없음 — 전체 텍스트 폴백 시도')
    const fallback = splitClaims(rawText)
    if (fallback.length > 0) {
      claims = fallback
      dbg(`  폴백 성공: ${claims.length}개 청구항 추출`)
    }
  }
