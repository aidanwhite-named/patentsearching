...
export class SearchEngine {
  ...
  async run(query: SearchQuery, onProgress?: ProgressCallback): Promise<SearchResult> {
    ...
    const parsedClaim = query.enrichedClaim
      ? enrichedToParsedClaim(query.enrichedClaim)
      : query.parsedClaim ?? parseClaim(query.queryText)
...
function enrichedToParsedClaim(enriched: EnrichedClaim): ParsedClaim {
  return {
    raw: enriched.originalClaim,
    elements: enriched.components.map((component, index) => ({
      id: String.fromCharCode(65 + index),
      text: [
        component.name,
        ...component.technicalConcepts,
        ...component.functionalRoles,
        ...component.synonyms,
        ...component.effects,
      ].filter(Boolean).join(' / '),
      importance: component.importance ?? 2,
      isEssential: (component.importance ?? 2) >= 2,
    })),
    broadTerms: [],
    technicalField: enriched.technicalDomain,
  }
}
