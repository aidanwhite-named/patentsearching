...
export type SourceType = 'patentsview' | 'kipris' | 'openalex' | 'bigquery'
...
export type SearchSource = 'patentsview' | 'kipris' | 'openalex' | 'bigquery'
...
export interface BigQueryUsage {
  bytesUsedThisMonth: number   // 이번 달 누적 처리 바이트
  resetMonth: string           // 'YYYY-MM' 형식 — 월이 바뀌면 자동 리셋
}

/** 월 무료 한도: 1TB = 1,000,000,000,000 bytes. 900GB까지만 허용 (10% 버퍼). */
export const BIGQUERY_FREE_TIER_BYTES = 1_099_511_627_776
export const BIGQUERY_SOFT_LIMIT_BYTES = Math.floor(BIGQUERY_FREE_TIER_BYTES * 0.9)
