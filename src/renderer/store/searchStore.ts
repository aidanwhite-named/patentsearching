...
const DEFAULT_SOURCES: SearchQuery['sources'] = ['openalex']
...
export const useSearchStore = create<SearchState>((set, get) => ({
  ...
  startSearch: () => {
    ...
    const enrichedClaims = wsState.enrichedClaims

    // 클레임 텍스트도 없고 쿼리도 없으면 patentStructure rawText 앞부분 폴백
    const effectiveQuery =
      queryText.trim() ||
      claimText.trim() ||
      wsState.patentStructure?.rawText.slice(0, 600).trim() ||
      ''
    if (!effectiveQuery) return
    const primaryEnrichedClaim = enrichedClaims.find((claim) => claim.isIndependent) ?? enrichedClaims[0]
    ...
    const query: SearchQuery = {
      ...
      language: 'both',
      enrichedClaim: primaryEnrichedClaim,
